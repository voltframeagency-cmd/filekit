        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-fk-lg border border-slate-200">
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400 block mb-1">
                {isFrench ? "Détails du fichier" : isGerman ? "Dateidetails" : isSpanish ? "Detalles del archivo" : "File Details"}
              </span>
            ...
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400 block mb-1">
                {isFrench ? "Métadonnées détectées" : isGerman ? "Erkannte Metadaten" : isSpanish ? "Metadatos detectados" : "Detected Metadata"}
              </span>
              <div className="flex flex-wrap gap-2">
                ...
                {metadata?.cameraMake && (
                  <span className="px-2 py-0.5 text-xs font-bold rounded bg-blue-100 text-blue-800">
                    {isFrench ? "Appareil :" : isGerman ? "Kamera:" : isSpanish ? "Cámara:" : "Camera:"} {metadata.cameraMake}
                  </span>
                )}
              </div>
            </div>
          ...
          {!outputUrl ? (
            ...
            >
              {loading
                ...
                : (isFrench
                    ...
                    : isSpanish
                    ? "Eliminar todos los metadatos EXIF y GPS"
            ...
            <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-fk-lg flex flex-col sm:flex-row items-center justify-between gap-3">
              ...
              >
                {isFrench ? "Télécharger la photo nettoyée" : isGerman ? "Bereinigtes Foto herunterladen" : isSpanish ? "Descargar foto limpia" : "Download Clean Photo"}
              </a>
            </div>
          )}
