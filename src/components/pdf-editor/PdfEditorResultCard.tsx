      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">
            {tr.browserPrivacyNotice(artifact.pageCount)}
          </p>
        </div>
      ...
      {artifact.splitArtifacts && artifact.splitArtifacts.length > 0 && (
        <div className="mt-6 pt-6 border-t border-slate-800">
          <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-3">
            {tr.splitOutputFiles(artifact.splitArtifacts.length)}
          </h4>
              ...
              <div key={sIdx} className="p-3 bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-[11px] text-slate-500">
                    {splitItem.pageCount} {splitItem.pageCount === 1 ? tr.pageWord : tr.pagesWord} • {formatBytes(splitItem.byteLength)}
                  </span>
                </div>
                <button
                  type="button"
