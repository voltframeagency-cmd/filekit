...
export interface ReverseOfficeWorkspaceProps {
  toolTitle: string;
  toolSlug: string;
  apiEndpoint: string;
  targetFormatExtension: string; // e.g. ".docx", ".xlsx", ".pptx"
  targetFormatName: string; // e.g. "Word Document (DOCX)", "Excel Spreadsheet (XLSX)", "PowerPoint Presentation (PPTX)"
  targetFormatBadge: string; // e.g. "DOCX", "XLSX", "PPTX"
}

export const ReverseOfficeWorkspace: React.FC<ReverseOfficeWorkspaceProps> = ({
  toolTitle,
  toolSlug,
  apiEndpoint,
  ...
  const executeServerConversion = async () => {
    ...
    try {
      ...
      setProgressStage("Reconstructing document layout and typography...");
      ...
      setProgressStage(`Packaging ${targetFormatBadge} container...`);
      ...
      setResult({
        ...
        fileName: sourceFile.name.replace(/\.pdf$/i, targetFormatExtension),
      });
    } catch (err: any) {
      ...
      {!sourceFile && (
        <div className="border-2 border-dashed border-slate-700 bg-slate-900/50 rounded-2xl p-8 sm:p-12 text-center flex flex-col items-center justify-center gap-4 hover:border-fk-primary transition-colors">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-slate-400">Convert PDF to editable {targetFormatName} with exact formatting.</p>
          </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col gap-6 shadow-xl">
          ...
          <div className="bg-slate-800/40 border border-slate-700/60 rounded-xl p-4 flex items-start gap-3">
            ...
            <div className="text-xs text-slate-300 flex flex-col gap-1 leading-relaxed">
              <span className="font-bold text-white">Ephemeral MicroVM Reconstruction</span>
              <span>
                Converting PDF vector structures into editable {targetFormatBadge} runs in an isolated container sandbox. Files are encrypted in transit and purged automatically from cloud memory immediately after conversion.
              </span>
            </div>
          </div>
          ...
          {result && (
            <div className="bg-emerald-950/40 border border-emerald-500/30 rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-bold text-white text-sm">Converted to {targetFormatBadge} Successfully</h4>
                  <p className="text-xs text-slate-400">
                    Reconstructed in {result.durationMs}ms • {(result.outputSizeBytes / 1024).toFixed(1)} KB • Ephemeral container purged
                  </p>
                </div>
              </div>
              <button
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-md w-full p-6 flex flex-col gap-5 shadow-2xl animate-in fade-in zoom-in-95">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              Converting PDF to editable <code>{targetFormatBadge}</code> requires isolated cloud structure reconstruction.
            </p>
