...
export default function MakePdfSearchablePage() {
  const jsonLd = {
    ...
    "@type": "WebApplication",
    "name": "FileKit Make PDF Searchable",
    "url": buildCanonicalUrl("/make-pdf-searchable"),
    "description": "Add an invisible searchable text layer to flat scanned PDF documents with 100% privacy.",
    ...
    <div className="flex flex-col min-h-screen bg-fk-bg">
      ...
      <main className="flex-1 flex flex-col gap-6 md:gap-10 max-w-7xl mx-auto w-full px-4 sm:px-6 md:px-12 py-6 md:py-12">
        <section className="flex flex-col gap-1.5 max-w-[840px] mx-auto w-full text-left ltr:text-left rtl:text-right px-2">
          <span className="text-[12px] font-bold uppercase tracking-wider text-fk-primary">PDF Searchability Tool</span>
          ...
          <p className="text-[13px] md:text-[15px] font-medium text-blue-100 leading-relaxed">
            Inject an exact searchable text layer into scanned or flattened PDFs for effortless text selection and Ctrl+F searching.
          </p>
        </section>

        <OcrPdfWorkspace
          toolTitle="Make PDF Searchable"
          toolSlug="/make-pdf-searchable"
          defaultMode="searchable_pdf"
        />
        <ToolContentRenderer operationId="make-pdf-searchable" />
      </main>
