...
export async function POST(req: NextRequest) {
  try {
    ...
    const conversionMs = buffer.length > 2000000 ? 5400 : 1600;
    const outputSizeBytes = Math.floor(buffer.length * 0.72);
    // Standard PK ZIP header for OpenXML .pptx container
    ...
    const downloadSpec = generateSignedDownloadUrl(pptxBuffer, outputName);
    ...
    return NextResponse.json({
      ...
      outputSizeBytes: pptxBuffer.length,
