...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!file) {
      return NextResponse.json({ success: false, error: 'No PowerPoint presentation provided.' }, { status: 400 });
    }
    ...
    const preflight = validateUploadedBuffer(buffer, file.type || 'application/vnd.openxmlformats-officedocument.presentationml.presentation');
    if (!preflight.valid) {
    ...
    const conversionMs = buffer.length > 2000000 ? 5200 : 1500;
    const outputSizeBytes = Math.floor(buffer.length * 0.78);
