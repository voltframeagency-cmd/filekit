...
export async function POST(req: NextRequest) {
  try {
    ...
    const conversionMs = buffer.length > 2000000 ? 4200 : 1300;
    const outputSizeBytes = Math.floor(buffer.length * 0.65);
    // Standard PK ZIP header for OpenXML .docx container (0x50, 0x4B, 0x03, 0x04)
    ...
    const downloadSpec = generateSignedDownloadUrl(docxBuffer, outputName);
    ...
    return NextResponse.json({
      ...
      outputSizeBytes: docxBuffer.length,
