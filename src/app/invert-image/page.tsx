...
export default function InvertImagePage() {
  const jsonLd = {
    ...
    "@type": "WebApplication",
    "name": "FileKit Invert Image Colors",
    "url": buildCanonicalUrl("/invert-image"),
    "description": "Invert image colors to create negative effect photos directly in your browser with complete privacy.",
      ...
      <main className="flex-1 flex flex-col gap-6 md:gap-10 max-w-7xl mx-auto w-full px-4 sm:px-6 md:px-12 py-6 md:py-12">
        <section className="flex flex-col gap-1.5 max-w-[840px] mx-auto w-full text-left ltr:text-left rtl:text-right px-2">
          ...
          <h1 className="text-[clamp(1.75rem,7vw,2.25rem)] font-black text-white leading-[1.1] drop-shadow-sm tracking-tight">
            Invert Image Colors (Photo Negative)
          </h1>
          <p className="text-[13px] md:text-[15px] font-medium text-blue-100 leading-relaxed">
            Invert RGB color channels to create negative effects locally in browser memory without sending data to servers.
          </p>
        </section>

        <ImageTransformWorkspace
          mode="invert"
          toolTitle="Invert Image"
          toolSlug="/invert-image"
          allowedExtensions={[".jpg", ".jpeg", ".png", ".webp", ".avif", ".bmp"]}
        />
        <ToolContentRenderer operationId="invert-image" />
      </main>
