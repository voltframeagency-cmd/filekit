...
async function createFixtures() {
  ...
  // 3. Digitally signed renderable PDF (contains AcroForm with SigFlags)
  const docSig = await PDFLib.PDFDocument.create();
  const pageSig = docSig.addPage([600, 400]);
  pageSig.drawText("FileKit Digitally Signed PDF Test Document", { x: 50, y: 300, size: 18 });
  ...
  // 4. Password-encrypted PDF simulation (%PDF- header + Encrypt dictionary)
  const docEnc = await PDFLib.PDFDocument.create();
  docEnc.addPage([600, 400]);
  ...
  console.log("Successfully created 5 deterministic PDF test fixtures in scripts/fixtures/");
}
