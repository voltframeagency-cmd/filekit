...
async function run() {
  console.log('Auditing Vietnamese (vi) across all', ALL_SLUGS.length, 'routes...');
  const leaksFound = [];
  let checked = 0;

  for (const slug of ALL_SLUGS) {
    const url = `http://localhost:3000/vi/${slug}`;
    try {
      ...
      if (leaks.length > 0) {
        console.warn(`[LEAK] /vi/${slug} ->`, leaks);
  ...
  // Also check homepage and /vi/all-tools
  for (const r of ['', 'all-tools']) {
    const url = `http://localhost:3000/vi/${r}`;
    try {
      const res = await fetch(url);
      const html = await res.text();
      const leaks = [];
  ...
  if (leaksFound.length === 0) {
    console.log('🎉 100% CLEAN! Zero English/raw key leaks detected in Vietnamese (vi).');
  } else {
